			...
			<div class="sp-simpleportfolio-item" data-groups='[<?php echo $groups; ?>]'>
				...
				<div class="sp-simpleportfolio-overlay-wrapper clearfix">
					...
					<?php if($this->params->get('thumbnail_type', 'masonry') == 'masonry') { ?>
						<img class="sp-simpleportfolio-img" src="<?php echo JURI::root(true) . '/images/spsimpleportfolio/' . $this->item->spsimpleportfolio_item_id . '/' . JFile::stripExt(JFile::getName($this->item->image)) . '_' . $sizes[$i] . '.' . JFile::getExt($this->item->image); ?>" alt="<?php echo $this->item->title; ?>">
					<?php } else if($this->params->get('thumbnail_type', 'masonry') == 'rectangular') { ?>
						<img class="sp-simpleportfolio-img" src="<?php echo JURI::root(true) . '/images/spsimpleportfolio/' . $this->item->spsimpleportfolio_item_id . '/' . JFile::stripExt(JFile::getName($this->item->image)) . '_600X400.' . JFile::getExt($this->item->image); ?>" alt="<?php echo $this->item->title; ?>">
					<?php } else { ?>
						<img class="sp-simpleportfolio-img" src="<?php echo JURI::root(true) . '/images/spsimpleportfolio/' . $this->item->spsimpleportfolio_item_id . '/' . JFile::stripExt(JFile::getName($this->item->image)) . '_600X600.' . JFile::getExt($this->item->image); ?>" alt="<?php echo $this->item->title; ?>">
					<?php } ?>
						...
						<div class="sp-vertical-middle">
							<div>
								<div class="sp-simpleportfolio-btns">
									...
									<?php } else { ?>
										<a class="btn-zoom" href="<?php echo JURI::root(true) . '/images/spsimpleportfolio/' . $this->item->spsimpleportfolio_item_id . '/' . JFile::stripExt(JFile::getName($this->item->image)) . '_600X400.' . JFile::getExt($this->item->image); ?>" data-featherlight="image"><?php echo JText::_('COM_SPSIMPLEPORTFOLIO_ZOOM'); ?></a>
									<?php } ?>
