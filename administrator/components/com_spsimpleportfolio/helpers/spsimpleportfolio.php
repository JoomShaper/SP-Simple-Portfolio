...
class SpsimpleportfolioHelper
{
	...
	public static function isPageBuilderIntegrated($item)
	{
		...
		if (PluginHelper::isEnabled('spsimpleportfolio', 'sppagebuilder'))
		{
			...
			if ($hasPage)
			{
				...
				$backend_link = 'index.php?option=com_sppagebuilder&view=editor&tmpl=componenet&extension=com_spsimpleportfolio&extension_view=item#/editor/' . $hasPage;

				$sefURIFrontEnd = str_replace('/administrator', '', $router->build($front_link));

				if ($lang_code && $lang_code !== '*' && $enable_lang_filter && $conf->get('sef'))
				{
					$sefURIFrontEnd = str_replace('/index.php/', '/index.php/' . $lang_code . '/', $sefURIFrontEnd);
				}
				elseif ($lang_code && $lang_code !== '*')
				{
					$sefURIFrontEnd = $sefURIFrontEnd . '&lang=' . $lang_code;
				}

				$output->front_url   = $sefURIFrontEnd;
				$output->backend_url = $backend_link;
			}
		}
	...
	/**
	 * Create PageBuilder Editor Link for Front-End and BackEnd
	 *
	 * @param   object $item	Item details
	 * @param   string $url		Front End / Back End Url
	 * @param   string $editor	Editor Type (Front End / Back End)
	 * @return  string
	 * @since   2.0.8
	 */
	public static function createPageBuilderLink($item,$url,$editor)
	{
		...
		{
			$output = "<a class='btn btn-small btn-success action-edit-width-sppb mt-3' target='_blank' href='#' data-id='" . $item->id . "' data-title='" . $item->title . "' data-editor='" . $editor . "'>";
		}

		$output .= ($editor === 'front') ? Text::_('COM_SPSIMPLEPORTFOLIO_TITLE_EDIT_WITH_SPPAGEBUILDER') : Text::_('COM_SPSIMPLEPORTFOLIO_TITLE_EDIT_WITH_SPPAGEBUILDER_BACKEND');
		$output .= '</a>';
		return $output;
	}
}
