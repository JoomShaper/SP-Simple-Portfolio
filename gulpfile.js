...
function cleanZip() {
  return src("./com_spsimpleportfolio_v2.2.2.zip", {
    read: false,
    allowEmpty: true,
  }).pipe(clean());
}
...
function copy_lang_site() {
  return src("./language/en-GB/en-GB.com_spsimpleportfolio.ini").pipe(
    dest("build/language/site/en-GB"),
  );
}
...
function copy_modules() {
  return src("./modules/mod_spsimpleportfolio/**/*.*").pipe(
    dest("build/modules/mod_spsimpleportfolio"),
  );
}

function copy_plugins() {
  return src("./plugins/finder/spsimpleportfolio/**/*.*").pipe(
    dest("build/plugins/finder/spsimpleportfolio"),
  );
}
...
function copy_plugins_lang() {
  ...
  ]).pipe(dest("build/plugins/finder/spsimpleportfolio/language"));
}
...
function makeZip() {
  return src("./build/**/*.*")
    .pipe(zip("com_spsimpleportfolio_v2.2.2.zip"))
    .pipe(dest("./"));
}
